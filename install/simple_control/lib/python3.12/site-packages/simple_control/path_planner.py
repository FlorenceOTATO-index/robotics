...
class PathPlanner(Node):
    ...
    def __init__(self):
        ...
        # safe_distance parameter
        self.declare_parameter("safe_distance", 1)
        self.safe_distance = self.get_parameter("safe_distance").get_parameter_value().integer_value
    ...
    def mainloop(self):
        ...
        if not self.have_plan:
            ...
            if (len(self.map) != 0) and (len(self.drone_position) == 2) and (len(self.goal_position) == 2):
                ...
                # Update to use a launch parameter instead of static value
                astar = AStarPlanner(safe_distance=self.safe_distance)
