...
class AStarPlanner:
    ...
    def plan(self, map_data, drone_position, goal_position):
        ...
        drone = (drone_position[0], drone_position[1])
        goal = (goal_position[0], goal_position[1])
        frontier.put((0, drone))
        came_from = {}
        cost_so_far = {}
        came_from[drone] = None
        cost_so_far[drone] = 0
        path = []
        ...
        while not frontier.empty():
            ...
            current_pri, current = frontier.get()
            # Check if the current node is the goal     
            if current == goal:
                break
            # For each neighbor of the current node
            for nxt in self.get_neighbors(current, map_data):
                # Compute the cost to traverse to that node
                next_node = (nxt[0], nxt[1]) 
